...
module Ardes::Test::ActiveRecord::Acts; end# :nodoc:

#
# To use this mixin do this
#
# class MyTest < Test::Rails::TestCase
#   test_has_handle :my_handle_model
# end
#
# And make sure your fixtures have at least two records as follows
# 
#  first:
#    id: 1
#    handle: first
...
#    handle: second
#
# (there can be more columns, and your handle column may be different)
#
module  Ardes::Test::ActiveRecord::Acts::Handle
  ...
  module ClassMethods
    def test_has_handle(target)
      include InstanceMethods
      self.class_eval do
        cattr_accessor :has_handle_class
        self.has_handle_class = target.to_s.classify.constantize
        alias_method :setup_without_has_handle, :setup
        alias_method :setup, :setup_with_has_handle
      end        
    end
  end
  ...
  module InstanceMethods
    ...
    def set_handle(obj, handle)
      accessor = obj.handle_column.to_s + "="
      obj.send(accessor, handle)
    end
    ...
    def test_ensure_that_find_works_with_handle_and_conditions
    ...
    def test_ensure_that_find_throws_record_not_found_with_bad_handle
      assert_raise(::ActiveRecord::RecordNotFound) { self.has_handle_class.find('not_there') }
    end

    def test_ensure_find_works_as_it_normally_should
      first = self.has_handle_class.find(1)
      assert_equal 1, first.id
...
Test::Rails::TestCase.class_eval { include Ardes::Test::ActiveRecord::Acts::Handle }
